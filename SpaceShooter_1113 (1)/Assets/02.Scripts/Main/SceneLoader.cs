...
public class SceneLoader : MonoBehaviour
{
    ...
    IEnumerator Start()
    {
        ...
        {
            yield return StartCoroutine(LoadScene(item.Key
                , item.Value));
        }
    ...
    IEnumerator LoadScene(string sceneName, LoadSceneMode mode)
    {
        ...
        Scene loadedScene = SceneManager
    ...
    IEnumerator Fade(float finalAlpha)
    {
        SceneManager.SetActiveScene(SceneManager
            .GetSceneByName("Level1"));
        fadeCg.blocksRaycasts = true;
        float fadeSpeed = Mathf.Abs(fadeCg.alpha - finalAlpha)
            / fadeDuration;
        while (!Mathf.Approximately(fadeCg.alpha, finalAlpha))
        {
            fadeCg.alpha = Mathf.MoveTowards(fadeCg.alpha
