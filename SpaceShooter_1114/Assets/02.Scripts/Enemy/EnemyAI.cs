...
public class EnemyAI : MonoBehaviour
{
    ...
    IEnumerator CheckState()
    {
        ...
        while (!isDie)
        {
            ...
            if (dist <= attackDist)
            {
                if (_enemyFOV.IsViewPlayer())
                {
                    _state = State.ATTACK;
                    //Debug.DrawRay(enemyTr.position + (Vector3.up * 1.8f), enemyTr.forward * 20.0f, Color.red);
                }
                else _state = State.TRACE;
            }
            else if (_enemyFOV.IsTracePlayer())
            {
                _state = State.TRACE;
            }
            else
